...
class Solution {
    ...
    Node copyList(Node head) {
        ...
        // Step 1: Insert new nodes after every original node.
        while (temp != null) {
            Node newNode = new Node(temp.data);  // Creating a new node
            newNode.next = temp.next;            // Linking newNode's next to temp's next
            temp.next = newNode;                 // Inserting newNode after temp
        ...
        while (temp != null) {
            if (temp.random != null) {
                temp.next.random = temp.random.next;  // Set newNode's random to temp.random's copy
            }
        ...
        while (temp != null) {
            ...
            if (copyTemp.next != null) {
                copyTemp.next = copyTemp.next.next;  // Link copied nodes together
            }
            temp = temp.next;           // Move to the next original node
            copyTemp = copyTemp.next;   // Move to the next copied node
        }
